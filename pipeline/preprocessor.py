...
# marker-pdf produces headings in various forms:
#   ## Item 1. Business
#   ## ITEM 1A. RISK FACTORS
#   **Item 7.** Management's Discussion...
#   Item 8.\nFinancial Statements
_SECTION_PATTERNS = [
    # Markdown heading: ## Item N[A-Z]?[.:]? ...
    re.compile(
        r"^#{1,4}\s*(ITEM|Item)\s+(\d+[A-Za-z]?)[.\s:]*(.*)$",
        re.MULTILINE,
    ),
    # Bold heading: **Item N[A-Z]?[.:]?** ...
    re.compile(
        r"^\*\*(ITEM|Item)\s+(\d+[A-Za-z]?)[.\s:]*\**\s*(.*)$",
        re.MULTILINE,
    ),
    # Plain line: ITEM N[A-Z]?. TITLE (all caps, standalone line)
    re.compile(
        r"^(ITEM)\s+(\d+[A-Za-z]?)[.\s:]+([A-Z][A-Z\s&,'\-()]+)$",
        re.MULTILINE,
    ),
]
...
def _split_into_sections(text: str) -> list[tuple[str, str, str]]:
    ...
    for pattern in _SECTION_PATTERNS:
        for m in pattern.finditer(text):
            item_num = m.group(2).strip()  # e.g. "1", "1A", "7"
            title_part = m.group(3).strip() if m.lastindex >= 3 else ""
    ...
    for pos, item_num, title in sorted(all_matches, key=lambda x: x[0]):
    ...
    for i, (pos, item_num, title) in enumerate(unique_matches):
        ...
        section_id = f"item_{item_num.lower().replace(' ', '_')}"
